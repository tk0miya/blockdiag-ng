...
// `getConnectors`/`getTextBox` are `null` for now - diamond.py's own
// connectors (this shape's own four points) and textbox (the inset box
// computed inline above) are added once connectors.ts exists (Step 18a).
export const diamondShape: NodeShape = { render: renderDiamondNode, getConnectors: null, getTextBox: null };
