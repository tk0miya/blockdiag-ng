...
describe("markSkippedEdges", () => {
  ...
  it("exercises the portrait-orientation flowchart-mode 'right-down' branch without changing the result", () => {
    // As with the landscape flowchart branch above, the flowchart-only
    // horizontal check (straight across from node1's row, before turning)
    // never finds anything occupying that row in this example - only the
    // landing-column check (also run in normal mode) does. This at least
    // confirms the branch runs without misbehaving.
    const diagram = route(
      "diagram { orientation = portrait; edge_layout = flowchart; A -> B; A -> D; D -> C; A -> C; }",
    );
    expect(skipped(diagram)).toEqual([
      ["A", "B", 0],
      ["A", "D", 0],
