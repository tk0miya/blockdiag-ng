...
import type { GroupItem, RelatedEdge } from "./related-nodes.js";
...
// this one).
function foldEndpoint(node: DiagramNode, level: number): { node: GroupItem; skip: boolean } {
  if (node.group === null || node.group.level < level) {
    return { node, skip: true };
  }
  let current: GroupItem = node;
  ...
  // `GroupItem`, which excludes the root `Diagram`.
