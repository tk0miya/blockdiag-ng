...
export type GroupItem = DiagramNode | NodeGroup;
...
export interface RelatedEdge {
  readonly node1: GroupItem;
  readonly node2: GroupItem;
...
export function getChildNodes(node: GroupItem, edges: readonly RelatedEdge[]): GroupItem[] {
  const children = new Set<GroupItem>();
...
export function getParentNodes(node: GroupItem, edges: readonly RelatedEdge[]): GroupItem[] {
  const parents = new Set<GroupItem>();
  for (const edge of edges) {
