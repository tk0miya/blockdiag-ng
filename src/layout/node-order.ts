...
function removeFirstMatch(circulars: GroupItem[][], target: readonly GroupItem[]): void {
  ...
  for (const c1 of [...circulars]) {
    for (let i = 0; i < circulars.length; i++) {
      ...
      if (!arraysEqual(c1, c2) && intersect.length === set1.size) {
        removeFirstMatch(circulars, c1);
        break;
      }
      if (!arraysEqual(c1, c2) && intersect.length > 0) {
        removeFirstMatch(circulars, c1);
        removeFirstMatch(circulars, c2);
...
// Moves the element at `movedIndex` to sit immediately after
// `anchorIndex`, both given by the caller (already computed there to
// decide which of the two to move) rather than re-derived here. Ported
// as-is: like `adjust_node_order()`'s own splice-equivalent calls, this
// doesn't check that `anchorIndex` actually precedes `movedIndex` - the
// x-column-based branches below call it either way, unguarded.
function moveAfter(nodes: GroupItem[], movedIndex: number, anchorIndex: number): void {
  const moved = nodes[movedIndex];
  nodes.splice(movedIndex, 1);
  nodes.splice(anchorIndex + 1, 0, moved);
}
...
export function adjustNodeOrder(
  ...
  for (const node of [...nodes]) {
    ...
    for (let i = 1; i < parents.length; i++) {
      ...
      if (idx1 < idx2) {
        moveAfter(nodes, idx2, idx1);
      } else {
        moveAfter(nodes, idx1, idx2);
      }
    }
