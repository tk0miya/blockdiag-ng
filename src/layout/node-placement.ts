...
function isRhombus(node1: GroupItem, node2: GroupItem, edges: readonly RelatedEdge[]): boolean {
  let a = node1;
  let b = node2;
  for (;;) {
    if (a === b) {
...
function setNodeYPos(
  ...
  let count = 0;
  let prevChild: GroupItem | null = null;
...
function calculateGroupSize(group: AnyGroup): void {
...
// `calculateGroupSize()`, so a group nested several levels deep can have
// its content flipped once by its own level's rotation and flipped again
// by an ancestor's.
